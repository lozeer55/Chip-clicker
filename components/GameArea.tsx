...
const ChipSVG: React.FC<{ onClick: (e: React.MouseEvent<HTMLElement>) => void; isBoostActive: boolean }> = ({ onClick, isBoostActive }) => (
    <div
        className={`relative w-64 h-64 md:w-72 md:h-72 cursor-pointer group animate-pulse-subtle ${isBoostActive ? 'animate-boost-glow' : ''}`}
        onClick={onClick}
    ...
    >
        <svg viewBox="0 0 256 256" xmlns="http://www.w3.org/2000/svg" className="w-full h-full transition-transform duration-100 group-active:scale-95 drop-shadow-[0_10px_8px_rgba(0,0,0,0.25)] group-hover:drop-shadow-[0_15px_15px_rgba(0,0,0,0.3)]">
            ...
            <rect x="88" y="88" width="80" height="80" stroke="#facc15" strokeWidth="12" strokeLinecap="round" strokeLinejoin="round" fill="#334155"/>
...
const ActiveBoostItem: React.FC<{ boost: ActiveBoost }> = ({ boost }) => {
    ...
    return (
        <div className="w-full p-2 bg-amber-500/10 border border-amber-400/20 rounded-lg shadow-sm backdrop-blur-sm">
            <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                    <div className="text-amber-400 flex-shrink-0"><LightningIcon /></div>
                    <div>
                        <h4 className="font-bold text-amber-300 text-sm leading-tight">{getBoostTitle()}</h4>
                        <p className="text-xs text-amber-400 leading-tight">
                            {boost.multiplier}x {boost.type === 'click_multiplier' ? 'Clicks' : 'CPS'}
                        </p>
                    </div>
                </div>
                <div className="text-xl font-bold text-amber-200 font-mono">{timeLeft}s</div>
            </div>
        </div>
    );
};
...
const GameArea: React.FC<GameAreaProps> = ({
  ...
  return (
    <div className="bg-slate-900/50 rounded-2xl shadow-inner border border-slate-700/50 p-6 flex flex-col items-center justify-between h-full">
        {/* Top Section */}
        <div className="text-center w-full">
            ...
            <div className="mt-4">
                <h2 className="text-5xl sm:text-6xl font-bold text-transparent bg-clip-text bg-gradient-to-b from-amber-300 to-amber-500 tracking-tighter font-mono" style={{textShadow: '0 0 15px rgba(252, 211, 77, 0.3)'}}>
        ...
        <div className="relative my-4 flex-grow flex flex-col items-center justify-center w-full">
            {isBoostActive && (
